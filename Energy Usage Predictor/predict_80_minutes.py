        ...
        for _, seq_length in enumerate(seq_lengths):
            ...
            for _, hidden_size in enumerate(hidden_sizes):
                ...
                for epoch in range(num_epochs):
                    ...
                    # Will need to make 8 predictions here, instead of just the one
                    # Summation of loss of predictions will be overall loss. 
                    ...
                    output = lstm(trainX)
                    loss = criterion(output, trainY[:, 0, :])
                    # Backpropogate
                    # loss.backward()
                    ...
                    for i in range(1, 8):
                        ...
                        # Calculate loss and add to overall loss
                        loss += criterion(output, trainY[:, i, :])


                        # Append prediction to the end of the train data
                        for j in range(len(trainX)):
                            test = trainX.clone()
                            intermediate = test[j][1:len(test[j])]
                            out = torch.cat((intermediate, output[j][None, ...]), axis=0)
                            trainX[j] = out
                            # trainX[j] = intermediate
                    ...
                    #     outputs.append(output)
                        ...
                        # for j in range(len(trainX)):
                        #     intermediate = trainX[j][1:len(trainX[j])]
                        #     intermediate = torch.cat((intermediate, output[j][None, ...]), axis=0)
                        #     trainX[j] = intermediate
